# sidecar verify — tier-rubric claim verification

주장(correctness · purity · grade · identity · 성능·정합성 claim)을 **LLM 자가판정으로
"맞다" 하지 말고**, 아래 6단계 티어 루브릭으로 등급을 매겨 그 배지를 **그대로(verbatim)**
보고한다. 코드/빌드/테스트가 실제로 도는지 검증하는 명령 실행은 `sidecar ci` 가 담당하고,
그 결과를 ledger 에 박제하려면 `sidecar verdict record <id> <cmd>` 를 쓴다.
...
- `sidecar verify` · `sidecar verify rubric` — 이 루브릭을 출력(레퍼런스).
- `sidecar verify fence "<claim>"` — 검증 안 된 주장을 ⚪ SPECULATION-FENCED 로 ledger 에 박제.
...
- 형식·수치로 확인할 수 있는 주장은 `sidecar ci`(명령 실행) + `sidecar verdict record`(박제)로
  실제 증거를 남긴 뒤 🟢/🔵 를 부여.
