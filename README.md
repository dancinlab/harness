...
| `changelog {add "<title>"\|list [N]\|render [N]\|prune --keep N\|--older-than D\|migrate}` | 이력 SSOT = **`CHANGELOG.jsonl`**(freeform `.md` 폐기 → newest-first JSONL · `{ts,title,body}`). `add`=append(body=stdin·ts=today) · `list`/`render`(markdown 뷰) · **`prune`**=오래된 엔트리 삭제(`--keep N` 또는 `--older-than D`) · `migrate`=md→jsonl 1회. **CHANGELOG-MISSING 게이트**(lint·pr-cycle·commit-gate)가 이제 `CHANGELOG.jsonl` staged 를 요구 | 매 사이클 doc + 이력 정리 |
