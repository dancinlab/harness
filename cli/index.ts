...
hook delegates (wire these into your agent's settings.json):
  ...
  changelog {add "<title>"|list [N]|render [N]|prune --keep N|--older-than D|migrate}   append-only history as CHANGELOG.jsonl (newest-first JSONL · ts+title+body) — add appends(body via stdin), prune deletes old entries(keep-N or age), render = markdown view, migrate = one-shot CHANGELOG.md→.jsonl. The CHANGELOG-MISSING gate now wants CHANGELOG.jsonl staged
