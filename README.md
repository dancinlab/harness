...
| 명령 | 역할 | hook 단계 |
|------|------|-----------|
| `pre bash` / `pre write` | **코드레벨 가드**(force-push{blind `--force`/`-f`/`+refspec` 차단 · `--force-with-lease`는 허용 · `# force-ok` 예외} · cloud-raw c11 · poll c19 · danger{`--no-verify`·`reset --hard`·`rm -rf` 루트만(`/`·`/*`·`~`·`$HOME`·`*` — `/tmp/x`·`~/foo` 등 특정경로 허용)·`curl\|sh`} · secret-literal c1 · handoff-scatter) → 그다음 config enforcement 정규식. 코드 가드는 config보다 먼저·끌 수 없음(profile 편집 무력화 방지) · 인라인 `# …-ok`/`// @secret-ok` 마커만 예외 | PreToolUse |
