...
| `lint [all\|fast]` | staged-L0 + 신선도 + **CHANGELOG 누락** + 수렴 누락 + **명령 desc 인지**(`SHADOW-DESC` warn — commands/*.md 가 ≤1400자 cap + `Triggers —` 절 보유) + **folder-guide**(`FOLDER-GUIDE-MISSING` block — staged 폴더에 CLAUDE.md) + **HELP 백틱**(`HELP-BACKTICK` block — cli/index.ts HELP 리터럴 내 이스케이프 안 된 백틱) 체크 | commit 전 (git pre-commit hook) |
| `naming audit [path] [--ing] [--gate]` | repo 전수 비-canonical 이름 감사 — git ls-files 트리에서 버전/복사/중복 접미사(`_v2`·`config copy`·`utils_old`…) 스캔. write-guard 가 못 본 **기존 backlog** 용(canonical-naming). `--gate`=위반 시 exit 1 · `--ing`=요약을 **그 repo 자기 ING** 보드에 add(cross-repo 없음) | repo 정리·감사 시 |
...
| `convergence {status\|recompute\|by-category\|scan\|due-check}` | incident 수렴 추적 + **`scan`** = 인라인 `@convergence` 재발방지 마커(c1) 검증(필수키 state·id + state enum) — `sidecar lint` 가 호출해 불량을 `CONVERGENCE-MALFORMED`(block)로 커밋 차단. 재발 신호(재발·stale·regression·again…)는 keyword 트리거가 `⟦CONVERGENCE-DUE id=…⟧` **캡처 토큰**+부채 출력 → well-formed 마커 작성 시 post-edit 자동 해소, 미해소면 **`due-check`** 가 Stop 에서 1회 환기(warn-only) | 버그 수정 후 · commit |
