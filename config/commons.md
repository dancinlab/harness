...
- do: 매 사이클 ① 문서(CHANGELOG append + ARCHITECTURE·ING 현행화 · README 는 손댄 김에 update-in-place·비강제) ② `sidecar pr-cycle` 로 검증된 main 머지 · 매 턴 파일 변동은 그 턴에 닫기(미완은 `wip:` 커밋) · 갱신 시 `🏛️ ARCHITECTURE`/`🔄 ING` 한 줄 보고 · 새 작업은 최신 base 에서 분기
...
## verdict-integrity — terminal 결론 전 측정경로부터 (negative 도 positive 와 동일 바)
- do: terminal/최종 결론(PASS·FAIL·천장) 전에 **측정·관측 경로 무결성부터** — reference/2nd-path/이전결과와 **발산하면 도구·하네스·환경·미완실행·미배선을 먼저 의심**, 정답지 대조(`reference-match`)로 artifact 배제한 뒤에만 terminal
- dont: negative/실패 결론을 측정확인 없이 terminal 박기(성공만 검증하고 실패는 rubber-stamp) · 발산을 곧장 대상 결함으로 단정(도구 먼저 의심) · 박았다 철회 반복
