...
| `lint [all\|fast]` | staged-L0 + 신선도 + **CHANGELOG 누락** + **convergence 레코드 위생**(`CONVERGENCE-MALFORMED` block — ARCHITECTURE.json `convergence.records` 의 id+state) + **명령 desc 인지**(`SHADOW-DESC` warn — commands/*.md 가 ≤1400자 cap + `Triggers —` 절 보유) + **folder-guide**(`FOLDER-GUIDE-MISSING` block — staged 폴더에 CLAUDE.md) + **commons do/dont**(`COMMONS-PROSE`/`COMMONS-NO-DODONT`/`COMMONS-DODONT-INCOMPLETE` block — 각 섹션 do+dont 둘 다) + **do/dont 길이**(`DODONT-LONG` block — **commons.md + 루트 CLAUDE.md** 의 do/dont 줄 >`dodontCap`(200) 신규/증가 · diff-aware · **서브폴더 CLAUDE.md**(folder-docs 로컬 가이드)는 자유양식이라 제외) + **HELP 백틱**(`HELP-BACKTICK` block — cli/index.ts HELP 리터럴 내 이스케이프 안 된 백틱) + **Blacksmith CI**(`CI-NON-BLACKSMITH` block — staged `.github/workflows/*.yml` 의 `runs-on:` 이 `blacksmith-*` 아니면 차단 · `${{ }}` 표현식 skip · config `ci.enforceRunner=false` 로 off · commons `blacksmith-ci`) 체크 | commit 전 (git pre-commit hook) |
