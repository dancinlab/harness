...
setup:
  ...
  switch {glm|claude|toggle|status|inject}   swap Claude Code's backend between the OFFICIAL Anthropic API and Z.AI GLM by
                                       rewriting the GLOBAL ~/.claude/settings.json env (ANTHROPIC_BASE_URL/AUTH_TOKEN + GLM model map) — Z.AI key from
                                       'secret get zai.api_key' (never on argv); env loads at startup so a restart is needed. inject = SessionStart 'MODEL : CLAUDE|GLM' readout
...
hook delegates (wire these into your agent's settings.json):
  ...
  architecture {inject|show|search <q>|lint|convergence ...}   surface repo-root ARCHITECTURE.json/.md (design SSOT) at SessionStart; search <q> = substring over id/name/role/detail → matching node ids+breadcrumb; lint = c4 tree hygiene + id presence/uniqueness; convergence {list|add|rm|edit} = 재발학습 store CRUD (id-keyed · add upsert · value/threshold '--value -'=stdin); convergence stop-check = Stop 훅 — 에이전트 출력의 재발-신호 키워드 스캔 → SSOT 기록 넛지(신호별 세션당 1회 · advisory · 패턴=config/convergence-triggers.json)
