...
| `architecture convergence {list\|add\|rm\|edit}` | 재발방지 학습 store CRUD — 학습은 `ARCHITECTURE.json` 의 `convergence.records[]`(id·state·value·threshold·source) 단일 SSOT 에 산다(구 `convergence` 명령·인라인 `@convergence` 마커 폐기). `add`=upsert(id-keyed) · `edit <id>` 부분 패치 · `rm <id>` · value/threshold 셸특수문자는 `--value -` stdin. 에이전트가 그 `source` 파일을 **Read/Write/Edit 할 때마다** PreToolUse 가 매칭 학습 주입(재도입 차단) · `sidecar lint` 가 레코드 well-formed(id+state) 강제 | 재발 학습 기록 · 파일 터치 시 자동 |
