---
description: /models {list|show|add|set|gate|feat|verify|prune|rm} — per-repo 모델 레지스트리 (단일 SSOT = ARCHITECTURE.json .models[]). 핵심 필드 + 3 유연축 gates{}(검증충족도)·progress(진행척도)·features[](특징). verify=sha256 무결성, prune=HF+sha 가드. Triggers — "모델 관리", "model add", "모델 등록", "검증 충족도", "/models".
argument-hint: "{list|show|add|set|gate|feat|verify|prune|rm}"
allowed-tools: Bash
---

# /models — per-repo 모델 레지스트리 (단일 SSOT)
...
## 서브커맨드
```
sidecar models list [--tier T] [--json]      모든 모델 (검증·진행·특징 한눈)
sidecar models show <id>                      한 모델 전체 JSON
sidecar models add  <id> [--arch --version --params --base --tier --sha --path --hf --visibility --progress --note --features a,b,c]
sidecar models set  <id> <field> <value...>   핵심 필드 갱신
sidecar models gate <id> <K=V> [K=V...]       검증 충족도 기록 (예: C1=pass C4=fail heldout='4/4 DESCENT')
sidecar models feat <id> <tag...> [--add]     특징 set (--add=추가)
sidecar models verify <id>                    weight sha256 재계산→기록값 대조 (무결성·드리프트 탐지)
sidecar models prune <id>                     HF 업로드 AND sha 일치 확인 후 로컬 weight 삭제 (registry 항목 보존, pruned=true)
sidecar models rm <id> --yes                  registry 항목 제거 (weight 파일은 그대로)
```
