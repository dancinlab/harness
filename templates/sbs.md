...
2. **한 번에 하나씩, 채팅으로 (NOT AskUserQuestion).** 비자명 문항은 easy 7요소로:
   ...
   **7요소 골격 = `sidecar easy` SSOT 래핑 (손으로 짜지 말 것).** 7요소 패턴 + 4종 ASCII 템플릿은
   `easy` 스타일이 소유한다 — 가리켜 APPLY 하되 복제 금지: `~/.sidecar/cli/styles/easy.<lang>.md`
   (없으면 repo `styles/easy.<lang>.md` · 기본 `ko` · 최신본 `ls -1 | sort -V | tail -1`). 라운드 prose
   (창의 채움)는 LLM 이, 골격/체크리스트는 SSOT 가 — 분리 유지. (`sidecar easy show` 로 스타일 본문 확인 가능.)
   라운드 후 styles 의 일반인-번역 체크리스트로 self-check(advisory · disambiguation 로직과 독립 · 항상 실행).
