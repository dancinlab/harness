...
| `model {list\|show\|add\|set\|gate\|feat\|verify\|prune\|rm}` | **모델 레지스트리** — 모델/ckpt 를 repo-root `ARCHITECTURE.json` top-level `models[]`(단일-doc SSOT)에 등록. 필드 id·arch·params·base·tier·sha256·path·hf·visibility + 3축(gates 검증충족도·progress 진행·features 특징). `verify`=weight sha256 재계산 대조(무결성) · `prune`=HF 업로드+sha 일치 확인 후 로컬 weight 삭제(registry 보존). writer=**byte-불변 top-level splice** · `dataset` 과 splice writer·flag 파서 공유(평행 레지스트리) | 모델 등록·조회 시 |
| `dataset {list\|show\|add\|set\|feat\|rm} [--lang ko\|en] [--register general\|sns]` | **데이터셋 레지스트리** — 코퍼스/데이터셋을 repo-root `ARCHITECTURE.json` top-level `datasets[]`(단일-doc SSOT · `models[]` 레지스트리의 데이터-짝)에 등록. 필드 id·repo_id(hf)·lang·register·rows·size·visibility·role·lang_verified·features[]·note·updated. `list`=4칸 lang×register 그리드 · writer=**byte-불변 top-level splice**(`datasets` 블록만 교체, 나머지 트리 byte-동일 → 거대 트리 reformat 0). `model` 과 동일 splice writer·flag 파서·idiom 으로 미러 | 데이터셋 등록·조회 시 |
...
| `architecture convergence {list\|for\|add\|rm\|edit\|stop-check}` | 재발방지 학습 store CRUD — 학습은 `ARCHITECTURE.json` 의 `convergence.records[]`(id·state·value·threshold·source) 단일 SSOT 에 산다(구 `convergence` 명령·인라인 `@convergence` 마커 폐기). `add`=**`--source` 필수**(학습은 그 파일에 키잉됨) + **id 자동 부여**(source 파일명 slug `<base>-<n>` · `--id` 로 upsert override) + **검토-우선 게이트**(동일 source 에 기존 레코드가 있으면 BLOCK — 무작정 append 금지 → 먼저 `convergence for <file>` 로 꺼내 보고, 같은 결함이면 `edit <id>` 로 갱신·정말 별개면 `--new` 로 의식적 추가 = 중복·미분류 누적 방지) · `for <file>` 그 파일의 학습 조회(읽기전용 · 터치 시 inject 와 동일 매칭) · `edit <id>` 부분 패치 · `rm <id>` · value/threshold 셸특수문자는 `--value -` stdin. **`stop-check`(Stop 훅 · 재발 트리거)** — **에이전트 자신의 마지막 응답**(user 입력 아님)을 재발-신호 키워드(`config/convergence-triggers.json` · 또/다시/실패/동일/재발/segfault/panic/🔴… · per-repo `.harness` override)로 스캔 → 매치 시 **enforce(`config.convergenceEnforce` · 기본 ON)**: 응답에 `🧬 CONVERGENCE` 계정 줄이 없으면 **`decision:block`** 으로 차단해 — 진짜 재발이면 `🧬 CONVERGENCE 기록: <id>`(+`convergence add`), 첫 발생·오탐이면 `🧬 CONVERGENCE: 해당 없음` 을 강제(ing/architecture stop-check 와 동형 response-marker 게이트 · `stop_hook_active` loop-guard 체인당 1블록 anti-wedge · enforce off 면 레거시 stderr warn). **키워드=넓은 그물, 정밀도는 에이전트 판단** — 마커가 곧 기록/기각 판정(자동 기록 아님). once-가드=**신호별 1회**(transcript+matched 키 · `convergence-nudge.json`) → 오탐 `또` 가 진짜 `segfault` 넛지를 소진 안 함 · 무매치/파싱실패 silent. 파일-터치 시 학습 주입(`convergenceForFile`)은 **활성**(`convergenceOnTouch` 기본 ON) — `preWrite` 가 그 파일에 키잉된 학습을 PreToolUse `additionalContext` 로 inject(stderr 아님 · 비-block 경로만 · 같은 결함 재도입 차단) · `sidecar lint` 가 레코드 well-formed(id+state) 강제 · `architecture inject` 스켈레톤은 records 제외(세부는 `convergence list` 온디맨드) | 재발 학습 기록 + Stop 트리거 |
...
| `pool {list\|add\|rm\|on <h> <cmd>\|status\|specs [h]}` | 호스트 로스터 + 원격 실행. `shared:false` 호스트는 **제한 호스트** — `allow` 프로젝트 컨텍스트 밖에선 `on` 차단(공용 컴퓨트로 못 씀). `on` 은 ssh 를 직접 spawn(argv)해 `cmd` 의 `$`/`$(...)` 가 로컬이 아니라 **원격 셸**에서 전개됨. `specs` 는 호스트별 코어/메모리/GPU 를 ssh 프로브해 로스터에 캐시(`list`·`status` 에 `〈12c · 30G · GPU:…〉` 인라인 표기) — 제한 호스트는 프로브하지 않음. `list`·`status` 는 **라이브 점유율** ⚡뱃지(`CPU 19%·RAM 9%·5.4/30G·GPU 0%·0/12GiB`)도 SSH 병렬 프로브(`status` 는 이 프로브로 도달여부까지 겸함 · 캐시 안 함 · RAM=total−available) | 원격 실행 · 자원 확인 시 |
...
| `pod {poll\|watch\|unwatch\|list}` | GPU pod **자동 폴링**(>=10분 cadence) — 발사 후 수동 ssh 확인을 canonical 명령으로 일반화. 모든 원격 op 은 `hexa cloud` 경유(`alive`·`exec`·`copy-from`·`rm` · 직접 ssh/curl 금지). `poll <id>` = one-shot **read-only**(alive→GPU-util/`--ssh-check`); `--teardown-on-done`/`--pull "<remote> <local>"` 있을 때만 회수 후 teardown(**pull-then-destroy**, a_fire_recover_complete). `watch <id> [--interval 600] [--cron]` = 레지스트리(`~/.sidecar/pod-watch.json`) 등록 + cadence 구동: `--cron`(시스템 crontab `*/10`) 또는 agent `ScheduleWakeup` fallback(기본 비파괴 · crontab write 불가 환경 자동 fallback). `unwatch`/`list` = 레지스트리 관리 | GPU pod 폴링 · 회수 |
...
- [ARCHITECTURE.json](ARCHITECTURE.json) — 사이드카 아키텍처 트리 SSOT (컬럼형 노드: name·role·id·detail · 각 노드 고유 kebab-case id=검색키, `sidecar architecture search <q>`). 사람용 뷰어는 [ARCHITECTURE.html](ARCHITECTURE.html) — 로컬은 `python3 serve.py`(서버 + 브라우저 자동 오픈), 원격은 raw.githack.com / GitHub Pages
...
> 🌳 **ARCHITECTURE.json 트리 위생 강제** (`architecture lint` · commons `single-doc`): 설계 트리는 **잘게 분해된 딱딱한 노드**로 유지된다 — `상세`/`역할` 셀이 `lint.archCellCap`(기본 **300**자 · config-driven · 0=off)을 넘거나(ARCH-BIG-CELL), ` · ` 로 묶인 항목이 `lint.archPiledMax`(6)를 넘으면(ARCH-PILED), 또는 history 성 키(previous/deprecated/…=ARCH-HISTORY)가 있으면 `sidecar lint`(pre-commit)가 **커밋을 막는다**(severity-map `ARCH-*=block`). 각 노드는 고유 kebab-case id 도 필수(부재/중복/형식 = ARCH-ID-MISSING/DUPE/FORMAT=block · 레거시 `slug`/한글키 read-fallback). 단, 이 게이트들은 canonical `name`/`children` 트리만 인식하므로, `name`(레거시 `이름`)-노드가 0인 비-canonical 스키마(예: `sections`/`blocks`/`title`)는 id 게이트가 **조용히 통과**하던 사각지대였다 — 이제 `ARCH-SCHEMA-UNRECOGNIZED`(warn)가 "id 게이트 비활성" 을 표면화한다(off-schema 설계문서가 id 강제를 몰래 우회 못 함). 큰 셀은 산문 leaf 가 아니라 **커널만 남기고**(산문·메커니즘·precedent 는 CHANGELOG/git/코드로) 또는 한 사실당 한 child 노드로 쪼갠다.
