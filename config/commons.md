...
정보 유지 — repo 에 있으면 매 사이클 현행화) ② **`harness pr-cycle`** 로 push→PR→**검증된 main 머지**(✅
<base> 머지 완료 — 검증됨 블록을 유저에게 그대로 보고). pr-cycle 의 doc-gate 가 CHANGELOG 또는 (존재 시)
ARCHITECTURE·README 미갱신이면 **거부**한다(`--no-doc` 는 진짜 문서 불필요할 때만).
