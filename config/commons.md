...
**상태변동 트리거(파일 변동과 무관)**: 파일이 하나도 안 바뀌어도 작업 **상태**가 바뀌면(=시작·진행단계
전환·블로커 발생·완료·다음 한 수 결정) 그 턴에 ING 를 갱신한다 — 코드는 그대로여도 "지금 뭐 하던 중"이
바뀌면 보드를 현행화(add/next/done). ING 는 커밋 불필요(빠른 갱신), 의미있는 코드 변동은 c14 로 닫는다.

## c12 — use the canonical CLI (not raw / 우회 habits)
...
## c16 — 벽을 만나면 돌파하라 (closed-negative ≠ 종착) · 벽 분류 먼저
...
- **(e) 투자 부족** — 스케일업(compute/data). 무거운 작업은 c17 대로 pool/`hexa cloud` 로 분산.
...
## c18 — 우회경로는 지시 전 작성 금지 (escape-hatch only on request)
