...
// This is the accumulation sibling of `convergence`: convergence records are recurrence-prevention
// learnings keyed to a `source` FILE (surfaced on touch); results are bench/experiment OUTCOMES
// keyed to a `subject` (what was measured) — NOT file-touched, no Stop-gate; they just pile up as
...
// records stay uniquely keyed to (kind, subject) and collision-free across rm/re-add.
export function nextResultId(kind: string, subject: string, existing: Set<string>): string {
  const slug = subject.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "result";
...
function loadResults(): ResultRecord[] {
  ...
  if (!found || !found.rel.endsWith(".json")) return [];
  try {
    const j = JSON.parse(readFileSync(found.path, "utf8")) as { results?: { records?: ResultRecord[] } };
...
function writeResults(records: ResultRecord[]): boolean {
  const found = pick();
  if (!found || !found.rel.endsWith(".json")) {
    loudFail("architecture result: ARCHITECTURE.json (JSON tree) required");
    return false;
  }
  let root: Record<string, unknown>;
  try {
    ...
    loudFail("architecture result: ARCHITECTURE.json is not valid JSON");
    return false;
  }
  const res = (root.results as { note?: string; records?: ResultRecord[] }) ?? {};
  res.records = records;
  root.results = res;
  writeFileSync(found.path, JSON.stringify(root, null, 2) + "\n");
  return true;
...
// commit-time validator — mirror lintConvergenceRecords: a record must carry a unique id,
// a valid kind, and a valid (3-stage) state, else the accumulated result is malformed.
export function lintResultRecords(): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
  loadResults().forEach((r, i) => {
    const at = r.id ? `id=${r.id}` : `record[${i}]`;
    if (!r.id) out.push(`result ${at} — missing required key: id`);
    else if (seen.has(r.id)) out.push(`result ${at} — duplicate id (records are id-keyed, update in place)`);
    else seen.add(r.id);
...
// accumulation store, mirroring convergenceVerb (id-keyed · add upserts on --id ·
// review-before-blind-append). value/metric shell-special chars: `--value -` reads stdin.
async function resultVerb(args: string[]): Promise<number> {
  const verb = args[0] ?? "list";
  const records = loadResults();

  if (verb === "list") {
    const kindFilter = flag(args, "kind");
    const shown = kindFilter ? records.filter((r) => r.kind === kindFilter) : records;
    if (!shown.length) return info(`result: no records${kindFilter ? ` (kind=${kindFilter})` : ""}`), 0;
    for (const r of shown) info(resultLine(r));
    info(`result: ${shown.length} record(s)${kindFilter ? ` (kind=${kindFilter})` : ` · ${records.length} total`}`);
    return 0;
  }

  // `for <subject>` — pull just THIS subject's accumulated results (the review step before
  // recording a new outcome / refining an existing one). Read-only, mirrors convergence `for`.
  if (verb === "for") {
    const subject = args[1];
    if (!subject) return info("usage: sidecar architecture result for <subject>"), 1;
    const hits = records.filter((r) => r.subject === subject);
    if (!hits.length) return info(`result: ${subject} 에 쌓인 결과 없음 (새 결과면 add)`), 0;
    info(`result — ${subject} 에 쌓인 벤치/실험 결과 ${hits.length}건 (같은 대상 재측정이면 edit <id> 로 갱신):`);
    for (const r of hits) info(resultLine(r));
    return 0;
  }

  if (verb === "add") {
    const kind = flag(args, "kind") ?? "experiment";
    ...
    const explicitId = flag(args, "id");
    const isNew = args.includes("--new");
    const same = records.filter((r) => r.kind === kind && r.subject === subject);
    if (same.length && !explicitId && !isNew) {
      loudFail(`result add: ${kind}/${subject} 에 이미 결과 ${same.length}건 — 무작정 추가 말고 먼저 검토하라:`);
      for (const r of same) info(resultLine(r));
      info("  · 같은 대상의 재측정/연장이면 → `architecture result edit <id> --value … [--state …] [--metric …]` 로 그 레코드 갱신(update-in-place)");
      info("  · 정말 별개의 새 런이면 → 같은 명령에 `--new` 를 붙여 의식적으로 추가");
      return 1;
    }
    const id = explicitId ?? nextResultId(kind, subject, new Set(records.map((r) => r.id ?? "")));
    const rec: ResultRecord = { id, kind, state: normalizeConvState(state), value, metric: flag(args, "metric") ?? "", subject };
    const idx = records.findIndex((r) => r.id === id);
    if (idx >= 0) records[idx] = rec;
    else records.push(rec);
    if (!writeResults(records)) return 1;
    ok(`result: ${idx >= 0 ? "updated" : "added"} ${id} (kind=${kind} · subject=${subject} · ${records.length} total)`);
    return 0;
  }
  ...
  if (verb === "edit") {
    const id = args[1] && !args[1].startsWith("--") ? args[1] : flag(args, "id");
    const rec = records.find((r) => r.id === id);
    if (!rec) return loudFail(`result edit: no record id=${id ?? "?"}`), 1;
    const kd = flag(args, "kind");
    if (kd !== undefined) {
    ...
    if (!writeResults(records)) return 1;
    ok(`result: edited ${rec.id}`);
    return 0;
  }

  if (verb === "rm") {
    const id = args[1];
    if (!id) return info("usage: sidecar architecture result rm <id>"), 1;
    const kept = records.filter((r) => r.id !== id);
    if (kept.length === records.length) return loudFail(`result rm: no record id=${id}`), 1;
    if (!writeResults(kept)) return 1;
    ok(`result: removed ${id} (${kept.length} remain)`);
    return 0;
  }
