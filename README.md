...
| `load {inject\|show}` | **매턴 자원 readout** — 답변 맨 위에 macOS 부하 한 줄 주입: CPU(load÷cores) · RAM(used%+커널 압박레벨) · swap, 3축 신호등. 어느 축이든 🔴/pressure≥warn 이면 `⚠️` + 무거운 작업 자제 가드. 부하로 죽는 맥은 CPU 아닌 MEMORY(compressor+swap)로 죽으므로 RAM·swap 동시 표시. `sysctl`+`vm_stat`만(가벼움), 비-macOS 무음 | UserPromptSubmit |
