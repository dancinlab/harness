...
// (`sidecar ing add [--to <repo>]`), NEVER ad-hoc HANDOFF.md / INBOX.md or
...
export function detectHandoffScatter(filePath: string): string | null {
  ...
  if (base === "HANDOFF.md" || base === "INBOX.md") {
    return `${base} is retired hand-off scatter — record it on the repo-root ING.jsonl instead: \`sidecar ing add <text>\` (cross-repo: \`--to <repo>\`).`;
  }
  if (/(^|\/)inbox\//.test(filePath) && filePath.endsWith(".md")) {
    return `inbox/*.md is the retired cross-repo scatter pattern — route it through ING.jsonl: \`sidecar ing add <text> --to <repo>\` (lands on the target repo's board, committed).`;
  }
  return null;
}
